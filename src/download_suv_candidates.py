...
SEARCH_QUERIES = [
    "toyota rav4 suv side view",
    "toyota land cruiser suv side view",
    "toyota highlander suv side view",
    "honda cr-v suv side view",
    "honda hr-v suv side view",
    "nissan qashqai suv side view",
    "nissan x-trail suv side view",
    "hyundai tucson suv side view",
    "hyundai santa fe suv side view",
    "kia sportage suv side view",
    "kia sorento suv side view",
    "volkswagen tiguan suv side view",
    "volkswagen touareg suv side view",
    "skoda kodiaq suv side view",
    "skoda karoq suv side view",
    "seat ateca suv side view",
    "renault kadjar suv side view",
    "renault captur suv side view",
    "peugeot 3008 suv side view",
    "peugeot 5008 suv side view",
    "citroen c5 aircross suv side view",
    "opel grandland suv side view",
    "ford kuga suv side view",
    "ford explorer suv side view",
    "jeep grand cherokee suv side view",
    "jeep compass suv side view",
    "range rover evoque suv side view",
    "range rover sport suv side view",
    "land rover discovery sport suv side view",
    "bmw x3 suv side view",
    "bmw x5 suv side view",
    "audi q3 suv side view",
    "audi q5 suv side view",
    "mercedes glc suv side view",
    "mercedes gle suv side view",
    "volvo xc40 suv side view",
    "volvo xc60 suv side view",
    "dacia duster suv side view",
    "mazda cx-5 suv side view",
    "subaru forester suv side view",
]
...
SUV_CANDIDATES_DIR = EXTERNAL_DATASETS_DIR / "suv_candidates"
SUV_SELECTED_DIR = EXTERNAL_DATASETS_DIR / "suv_selected"
...
def prepare_directories():
    SUV_CANDIDATES_DIR.mkdir(parents=True, exist_ok=True)
    SUV_SELECTED_DIR.mkdir(parents=True, exist_ok=True)


def clear_previous_candidates():
    removed_count = 0

    for file_path in SUV_CANDIDATES_DIR.glob("suv_candidate_*"):
...
def count_candidate_images():
    return sum(
        1
        for file_path in SUV_CANDIDATES_DIR.iterdir()
...
def download_candidates_for_query(ddgs_class, query, known_hashes):
    ...
    for result in search_results:
        ...
        file_name = f"suv_candidate_{slug}_{saved_index:06d}{extension}"
        target_path = SUV_CANDIDATES_DIR / file_name
...
def main():
    print("SUV aday gorsel indirme islemi baslatiliyor...")
    prepare_directories()
    ...
    print(f"Onceki suv_candidate_* dosya sayisi temizlendi: {removed_count}")
    print(f"Candidate klasoru: {SUV_CANDIDATES_DIR}")
    print(f"Selected klasoru: {SUV_SELECTED_DIR}")
    ...
    print(f"suv_candidates toplam aday gorsel sayisi: {count_candidate_images()}")
    print("suv_selected klasoru manuel secim icin hazir.")
    print("Not: Bu script dataset/raw/SUV klasorune dosya kopyalamaz.")
