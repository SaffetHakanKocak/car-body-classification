...
SEARCH_QUERIES = [
    "open wheel race car real photo",
    "formula 1 car real photo",
    "f1 car side view real photo",
    "formula racing car real photo",
    "indycar race car real photo",
    "single seater race car real photo",
    "formula car side view",
    "open wheel racing car side view",
    "f1 race car track photo",
    "formula 3 car real photo",
    "formula 2 car real photo",
    "indycar side view",
    "single seater formula car",
    "open wheel motorsport car",
    "formula race car front view",
    "formula race car rear view",
]
...
OPEN_WHEEL_CANDIDATES_DIR = EXTERNAL_DATASETS_DIR / "open_wheel_candidates"
OPEN_WHEEL_SELECTED_DIR = EXTERNAL_DATASETS_DIR / "open_wheel_selected"
...
def prepare_directories():
    OPEN_WHEEL_CANDIDATES_DIR.mkdir(parents=True, exist_ok=True)
...
def clear_previous_candidates():
    ...
    for file_path in OPEN_WHEEL_CANDIDATES_DIR.glob("open_wheel_candidate_*"):
...
def count_candidate_images():
    return sum(
        1
        for file_path in OPEN_WHEEL_CANDIDATES_DIR.iterdir()
...
def download_candidates_for_query(ddgs_class, query, known_hashes):
    ...
    for result in search_results:
        ...
        file_name = f"open_wheel_candidate_{slug}_{saved_index:06d}{extension}"
        target_path = OPEN_WHEEL_CANDIDATES_DIR / file_name
...
def main():
    print("OPEN_WHEEL aday gorsel indirme islemi baslatiliyor...")
    prepare_directories()
    ...
    print(f"Onceki open_wheel_candidate_* dosya sayisi temizlendi: {removed_count}")
    print(f"Candidate klasoru: {OPEN_WHEEL_CANDIDATES_DIR}")
    print(f"Selected klasoru: {OPEN_WHEEL_SELECTED_DIR}")
    ...
    print(f"open_wheel_candidates toplam aday gorsel sayisi: {count_candidate_images()}")
    print("open_wheel_selected klasoru manuel secim icin hazir.")
    print("Not: Bu script dataset/raw/OPEN_WHEEL klasorune dosya kopyalamaz.")
